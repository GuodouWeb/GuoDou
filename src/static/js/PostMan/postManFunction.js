...
function sendBtn(proxy) {
  ...
  request(
    JSON.stringify({
      ...
      header: rq_info.headers,
      params: rq_info.params,
    })
  )
    .then((res) => {
      ...
      if (typeof res.data.data.response === "object") {
        proxy.$refs.res_Editor.setJSONVal(
...
function SetResHeader(res) {
  ...
  headerMap.forEach((V, K) => {
    res_info.headers.push({ key: K, value: V });
  });
  cookiesMap.forEach((V, K) => {
    res_info.Cookis.push({ key: K, value: V });
  });
}
